...
from .models import User, Campaign, CampaignMedia, Donation, News
...
class CampaignSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Campaign
        fields = [
            'id', 'title', 'short_description', 'description', 'target_amount',
            'current_amount', 'status', 'created_by', 'created_at', 'updated_at',
            'media', 'progress_percentage', 'moderation_notes'
        ]
...
class CampaignCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for idx, media_file in enumerate(media_files[:6]):  # Limit to 6 files
            media_type = 'video' if hasattr(media_file, 'content_type') and media_file.content_type.startswith('video/') else 'image'
