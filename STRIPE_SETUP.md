...
2. Ensure `STRIPE_ONBOARDING_RETURN_URL` and `STRIPE_ONBOARDING_REFRESH_URL` are publicly reachable URLs that redirect users back to your frontend dashboard.
3. If you use different environments (development, staging, production), configure separate URLs for each.
...
- Confirm `STRIPE_ONBOARDING_RETURN_URL` and `STRIPE_ONBOARDING_REFRESH_URL` are configured and match the URLs registered in the Stripe Dashboard.
